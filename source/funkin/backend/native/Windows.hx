...
#if windows
import funkin.backend.native.NativeAPI.FileAttribute;
import funkin.backend.native.NativeAPI.MessageBoxIcon;
import funkin.backend.native.NativeAPI.MessageBoxReturnValue;

/**
 * @see https://github.com/FNF-CNE-Devs/CodenameEngine/blob/main/source/funkin/backend/utils/native/Windows.hx
 */
...
class AudioFixClient : public IMMNotificationClient {
	...
	HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(
		...
		LPCWSTR pwstrDeviceId)
	{
		::funkin::backend::Main_obj::audioDisconnected = true;
		return S_OK;
	};
};
...
class Windows {
	...
	public static function registerAudio() {
		funkin.backend.Main.audioDisconnected = false;
	}
	...
	@:functionCode('
		return MessageBox(GetActiveWindow(), message, caption, options | MB_SETFOREGROUND);
	')
