...
/**
 * A modified `FunkinSprite` that supports filters.
 * The name's pretty much self-explanatory.
 * 
 * @see https://github.com/FunkinCrew/Funkin/blob/main/source/funkin/graphics/FlxFilteredSprite.hx
 */
// shittily porting this to the engine because i need it for SCS
@:nullSafety
@:access(openfl.geom.Rectangle)
@:access(openfl.filters.BitmapFilter)
@:access(flixel.graphics.frames.FlxFrame)
class FilteredSprite extends FunkinSprite {
    @:noCompletion static var _filterRect:Rectangle = new Rectangle();
    ...
    @:noCompletion var changedFrame:Bool = false;
	...
	#end
	{
		...
		camera.drawPixels((filtered) ? _blankFrame : frame, framePixels, matrix, colorTransform, blend, antialiasing, shader);
	}
    ...
    function filterFrame() {
        ...
        if (filters != null && filters.length > 0) {
            ...
            // fixes an issue where animations just make the filters
            // A. disappear
            // or B. shit themselves and look very weird
            // TODO: this works but is laggy as shit
            @:nullSafety(Off)
            if (changedFrame && _blankFrame != null) {
                if (_blankFrame.parent != null) {
                    _blankFrame.parent.destroy();
                    _blankFrame.parent = null;
                    @:privateAccess {
                        if(_filterBmp1.__texture != null)
                            _filterBmp1.__texture.dispose();

                        if(_filterBmp2.__texture != null)
            ...
            if (_blankFrame.parent == null
                || _flashRect.width > _blankFrame.parent.width
                || _flashRect.height > _blankFrame.parent.height) {
                if (_blankFrame.parent != null) {
                    _blankFrame.parent.destroy();
                    @:privateAccess {
                        if(_filterBmp1.__texture != null)
