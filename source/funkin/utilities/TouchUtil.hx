...
class TouchUtil {
     ...
     * Returns `0` on mobile, use SwipeUtil on mobile instead!
     */
    public static var wheel(get, never):Float;
	...
	public static function overlaps(?object:FlxBasic, ?camera:FlxCamera):Bool {
		if (object == null || touch == null)
			return false;

		return touch.overlaps(object, camera ?? object.camera);

		// return false; // how was this even compiling -swordcube
	}
	...
	public static function pressAction(?object:FlxBasic, ?camera:FlxCamera, useOverlapsComplex:Bool = true):Bool {
		if (TouchUtil.touch == null || (TouchUtil.touch != null && TouchUtil.touch.ticksDeltaSincePress > 200))
			return false;
    ...
    @:noCompletion
    static function get_wheel():Float {
        return 0;
    }
	#else
	@:noCompletion
