...
vec4 effect(vec4 color, Image texture, vec2 textureCoords, vec2 screenCoords) {
    ...
    // comet premultiplies alphas before sending it to render, so we want to accomodate header. This fixes some antialiased edges appearing darker
    vec3 unpremultipliedColor = pixel.a > 0.0 ? pixel.rgb / pixel.a : pixel.rgb;

	// Apply effects to the unpremultiplied color
	vec3 outColor = applyHSBCEffect(unpremultipliedColor);

	return vec4(outColor * pixel.a, pixel.a) * color;
}