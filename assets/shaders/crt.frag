...
// converted to love2d shader by hand by me!!! (swordcube!!!)
...
vec4 effect(vec4 color, Image texture, vec2 textureCoords, vec2 screenCoords) {
	...
	vec3 baseCol = Texel(texture, uv).rgb;
	vec3 col = baseCol;
	...
	col *= 1.0 - 0.65 * vec3(clamp((mod(love_PixelCoord.x, 2.0) - 1.0) * 2.0, 0.0, 1.0));

	// Remove the next line to stop cross-fade between original and postprocess
	col = mix(baseCol, col, percent);
	return vec4(col, 1.0);
}