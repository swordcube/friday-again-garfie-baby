...
baseAtlas.hardBake = function(self, ...)
    ...
    for _, value in pairs(self.images) do
        value.image:release()
    end
    self.images = nil
    self.ids = nil
...
baseAtlas.toFrameCollection = function(self)
    local ids = self.ids -- we wanna hold on to these
    if not self._hardBake then
        self:hardBake()
    end
